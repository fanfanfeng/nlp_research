# create by fanfan on 2020/1/9 0009
...
def encode_pieces(sp_model,text,return_unicode=True,sample=False):
    ...
    for piece in pieces:
        ...
        if len(piece) > 1 and piece[-1] == ',' and piece[-2].isdigit():
            cur_pieces = sp_model.EncodeAsPieces(six.ensure_binary(pieces[:-1]).replace(SPIECE_UNDERLINE,b""))
...
class FullTokenizer(object):
    ...
    def convert_tokens_to_ids(self,tokens):
        if self.sp_model:
            tf.logging.info("using sentence piece tokenizer.")
...
class BasicTokenizer(object):
    ...
    def tokenizer(self,text):
        ...
        for token in orig_tokens:
            ...
            split_tokens.extend(self._run_strip_accents(token))
