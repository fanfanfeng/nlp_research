# create by fanfan on 2019/2/18 0018
import collections
import csv
import os
from LangueModel.BERT import modeling
from LangueModel.BERT import  optimization
...
flags.DEFINE_integer("eval_batch_size",8,"Total batch size for predict.")
...
def file_based_convert_example_to_features(examples,label_list,max_seq_length,tokenizer,output_file):
    ...
    for (ex_index,example) in enumerate(examples):
        ...
        def create_int_feataure(values):
            f = tf.train.Feature(int64_list = tf.train.Int64List(value=list(values)))
            return f

        features = collections.OrderedDict()
        features['input_ids'] = create_int_feataure(feature.input_ids)
        features['input_mask'] = create_int_feataure(feature.input_mask)
        features['segment_ids'] = create_int_feataure(feature.segment_ids)
        features['label_ids'] = create_int_feataure([feature.label_id])
        features['is_real_example'] = create_int_feataure([int(feature.is_real_example)])
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        tensors_to_log = {'train_loss':'loss/Mean:0'}
    ...
    if FLAGS.do_eval:
        eval_examples = processor.get_dev_example(FLAGS.data_dir)
