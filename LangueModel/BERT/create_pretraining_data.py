# create by fanfan on 2018/12/26 0026
import random
import tensorflow as tf
from LangueModel.BERT import tokenization
import collections
import os

########## default params ############
bert_root = os.path.abspath(os.path.dirname(__file__))
input_file = os.path.join(bert_root,'data/sample_text.txt')
output_file = os.path.join(bert_root,'data/tf_examples.tfrecord')
vocab_file =  os.path.join(bert_root,'model/vocab.txt')

flags = tf.flags
FLAGS = flags.FLAGS
flags.DEFINE_string("input_file",input_file,"Input raw text file (or comma-separated list of files).")
flags.DEFINE_string("output_file",output_file,"Output TF example file (or comma-separated list of files).")
...
flags.DEFINE_float('short_seq_prob',0.1,"Probability of creating sequences which are shorter than the "
    "maximum length.")
...
def write_instance_to_example_files(instances,tokenizer,max_seq_length,max_predictions_pre_seq,output_files):
    ...
    for (inst_index,instance) in enumerate(instances):
        ...
        while len(masked_lm_positions) < max_predictions_pre_seq:
...
def create_training_instance(input_files,tokenizer,max_seq_length,dupe_factor,short_seq_prob,masked_lm_prob,max_predictions_per_seq,rng):
    ...
    for input_file in input_files:
        with tf.gfile.GFile(input_file,'r') as reader:
            while True:
                ...
                # Empyt lines are used as document delimiters
                if not line:
...
def main(_):
    ...
    instances = create_training_instance(input_files,tokenizer,FLAGS.max_seq_length,FLAGS.dupe_factor,FLAGS.short_seq_prob,FLAGS.masked_lm_prob,FLAGS.max_predictions_per_seq,rng)
