# create by fanfan on 2019/2/13 0013
import os
...
flags.DEFINE_integer("eval_batch_size",8,'Total batch size for training.')

flags.DEFINE_float('learning_rate',5e-5,'The initial learning rate for Adam.')

flags.DEFINE_integer('num_train_steps',100000,'Number of training steps. ')

flags.DEFINE_integer('num_warmup_steps',10000,'Number of warnup steps.')
    ...
    def model_fn(features,labels,mode,params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(masked_lm_example_loss,masked_lm_log_probs,masked_lm_ids,
                          masked_lm_weights,next_sentence_example_loss,next_sentence_log_probs,next_sentence_labels):
...
def gather_indexes(sequence_tensor,positions):
    ...
    flat_positons = tf.reshape(positions + flat_offsets,[-1])
    flat_sequence_tensor = tf.reshape(sequence_tensor,[batch_size * seq_length,width])
    output_tensor = tf.gather(flat_sequence_tensor,flat_positons)
...
def main(_):
    ...
    if FLAGS.do_eval:
        tf.logging.info("***** Runing evaluation ******")
