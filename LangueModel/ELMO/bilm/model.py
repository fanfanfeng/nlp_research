# create by fanfan on 2018/12/3 0003
import numpy as np
import tensorflow as tf
import h5py
import jieba
import  json
from LangueModel.ELMO.bilm.data import UnicodeCharsVocabulary,Batcher,InvalidNumberOfCharacters
...
class BidirectionalLanguageModel(object):
    def __init__(self,
        ...
        if not use_character_inputs:
            if embedding_weight_file is None:
                raise ValueError("embedding_weight_file is required input with not use_character_inputs")
    ...
    def _build_ops(self,lm_graph):
        with tf.control_dependencies([lm_graph.update_state_op]):
            ...
            n_lm_layer = len(lm_graph.lstm_outpus['forward'])
...
class BidirectionalLanguageModelGraph(object):
    '''
        ...
        '''
    def __init__(self,options,weight_file,ids_placeholder,use_character_inputs=True,embedding_weight_file =None,max_batch_size = 128):
    ...
    def _build_word_char_embeddings(self):
        ...
        with tf.device('/cpu:0'):
            ...
            self.char_embeddings = tf.nn.embedding_lookup(self.embedding_weights,self.ids_placeholder)
        ...
        embedding = make_convolutions(self.char_embeddings)

        # for highway and projection layers
        n_highway = cnn_options.get("n_hightway")
        ...
        def high(x,ww_carry,bb_carry,ww_tr,bb_tr):
            carray_gate = tf.nn.sigmoid(tf.matmul(x,ww_carry) + bb_carry)
            transform_gate = tf.nn.relu(tf.matmul(x,ww_tr) + bb_tr)
            return carray_gate * transform_gate + (1.0 - carray_gate) * x
    ...
    def _build_word_embeddings(self):
        ...
        # the word embeddings
        with tf.device('/cpu:0'):
            self.embedding_weights = tf.get_variable('embedding',[self._n_tokens_vocab,projection_dim],dtype=DTYPE)
    ...
    def _build_lstms(self):
        ...
        for direction in ['forward','backward']:
            if direction == 'forward':
                ...
                layer_input = tf.reverse_sequence(self.embedding,sequence_lengths,seq_axis=1,batch_axis=0)

            for i in range(n_lstm_layers):
                if projection_dim < lstm_dim:
                    # are projecting down output
                    lsmt_cell = tf.nn.rnn_cell.LSTMCell(lstm_dim,num_proj=projection_dim,cell_clip=cell_clip,proj_clip=proj_clip)
                else:
                ...
                if use_skip_connections:
                    ...
                    if i == 0:
                        ...
                        # add skipp connection
                        lsmt_cell = tf.nn.rnn_cell.ResidualWrapper(lstm_cell)
                ...
                init_states = [
                    tf.Variable(tf.zeros([self._max_batch_size,dim]),trainable=False) for dim in lstm_cell.state_size
                ]
                batch_init_states = [state[:batch_size,:] for state in init_states]
                if direction == 'forward':
                    i_direction = 0
                else:
                    i_direction = 1
                ...
                with tf.variable_scope(variable_scope_name):
                    layer_output,final_state = tf.nn.dynamic_rnn(lstm_cell,layer_input,sequence_lengths=sequence_lengths,init_states = tf.nn.rnn_cell.LSTMStateTuple(*batch_init_states))
...
def dump_token_embeddings(vocab_file,options_file,weight_file,outfile):
    ...
    with tf.Session(config=config) as sess:
        ...
        for k in range(n_tokens):
            ...
            char_ids = batcher.batch_sentences([[token]])[0,1,:].reshape(1,1,-1)
            embeddings[k,:] = sess.run(embedding_op,feed_dict={ids_placeholder:char_ids})

    with h5py.File(outfile,'w') as fout:
        ds = fout.create_dataset('embedding',embeddings.shape,dtype='float32',data=embeddings)

def dump_bilm_embeddings(vocab_file,dataset_file,options_file,weight_file,outfile):
    with open(options_file,'r') as fin:
        options = json.load(fin)
    max_word_length = options['char_cnn']['max_characters_per_token']
    voacb = UnicodeCharsVocabulary(vocab_file,max_word_length)
    batcher = Batcher(vocab_file,max_word_length)
