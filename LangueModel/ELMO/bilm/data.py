# create by fanfan on 2018/11/27 0027
import glob
import random
import numpy as np
from typing import  List

from utils.text_util import chinese_tokenizer
...
class UnicodeCharsVocabulary(Vocabulary):
    ...
    def __init__(self,filename,max_word_length,**kwargs):
        ...
        self.bos_chars = _make_bos_eos(self.bow_char)
    ...
    def max_word_length(self):
     ...
     # chinese word can also be 0-255
...
class Batcher(object):
    ...
    def batch_sentences(self,sentences:List[List[str]]):
        ...
        X_char_ids = np.zeros((n_sentences,max_length,self._max_token_length),dtype=np.int64)
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            while cur_pos < num_steps:
                if cur_steam[i] is None or len(cur_steam[i][0]) <= 1:
                    try:
                        cur_steam[i] = list(next(generator))
                    except StopIteration:
                        no_more_data = True
                        break
                how_many = min(len(cur_steam[i][0]) -1,num_steps - cur_pos)
                next_pos = cur_pos + how_many

                inputs[i,cur_pos:next_pos] = cur_steam[i][0][:how_many]
                if max_word_length is not None:
                    char_inputs[i,cur_pos:next_pos] = cur_steam[i][1][:how_many]
                targets[i,cur_pos:next_pos] = cur_steam[i][0][1:how_many + 1]
                cur_pos = next_pos

                cur_steam[i][0] = cur_steam[i][how_many:]
                if max_word_length is not None:
                    cur_steam[i][1] = cur_steam[i][1][how_many:]
        if no_more_data:
            break
...
class LMDataset(object):
    """
        ...
        """
    def __init__(self,filepattern,vocab,reverse=False,test=False,shuffle_on_load=True):
        '''
    ...
    def _load_shard(self,shard_name):
        ...
        for sentence in sentences:
            temp = chinese_tokenizer(sentence)
            id = self._vocab.encode(temp,self._reverse)
            chars_id = self._vocab.encode_chars(temp,self._reverse)
...
class BidirectionalLMDataSet(object):
    def __init__(self,filepattern,vocab,test=False,shuffle_on_load=False):
        '''
            ...
            '''
        self._data_forward = LMDataset(filepattern,vocab,reverse=False,test=test,shuffle_on_load=shuffle_on_load)
        self._data_reverse = LMDataset(filepattern,vocab,reverse=True,test=test,shuffle_on_load=shuffle_on_load)

    def iter_batches(self,batch_size,num_steps):
        max_word_length = self._data_forward.max_word_length
        for X,Xr in zip(_get_batch(self._data_forward.get_sentence(),batch_size,num_steps,max_word_length),
