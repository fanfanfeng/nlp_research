# create by fanfan on 2018/12/5 0005
import os
import time
import json
import re
...
from LangueModel.ELMO.bilm.data import Vocabulary,UnicodeCharsVocabulary,InvalidNumberOfCharacters
...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        self.token_ids = tf.placeholder(DTYPE_INT,shape=(batch_size,unroll_steps),name='token_ids')

        # the word embeddings
        with tf.device("/cpu:0"):
            self.embedding_weights = tf.get_variable('embeddings',[n_tokens_vocab,projection_dim],dtype=DTYPE)
        ...
        if self.bidirectional:
            self.token_ids_reverse = tf.placeholder(DTYPE_INT,shape=(batch_size,unroll_steps),name='token_ids_reverse')
            with tf.device('/cpu:0'):
    ...
    def _build_word_char_embeddings(self):
        ...
        if n_chars != 261:
            raise InvalidNumberOfCharacters("Set n_characters=261 for training see the README.md")
        ...
        self.tokens_characters = tf.placeholder(DTYPE_INT,shape=(batch_size,unroll_steps,max_chars),name='tokens_characters')

        # the character embeddings
        with tf.device('/cpu:0'):
            self.embedding_weights = tf.get_variable('char_embed',[n_chars,char_embed_dim],dtype=DTYPE,initializer=tf.random_uniform_initializer(-1.0,1.0))

            # shape(batch_size,unroll_steps,max_chars,embed_dim)
            self.char_embedding = tf.nn.embedding_lookup(self.embedding_weights,self.tokens_characters)

            if self.bidirectional:
                self.tokens_characters_reverse = tf.placeholder(DTYPE_INT,shape=(batch_size,unroll_steps,max_chars),name='tokens_characters_reverse')
                self.char_embedding_reverse = tf.nn.embedding_lookup(self.embedding_weights,self.tokens_characters_reverse)
            ...
            with tf.variable_scope("CNN",reuse=reuse) as scope:
                ...
                for i,(width,num) in enumerate(filters):
                    if cnn_options['activation'] == 'relu':
                        ...
                        w_init = tf.random_normal_initializer(mean=0.0, stddev=np.sqrt(1.0 / (width * char_embed_dim)))

                    w = tf.get_variable('W_cnn_%s' % i,[1,width,char_embed_dim,num],initializer=w_init,dtype=DTYPE)
                    b = tf.get_variable("b_cnn_%s" % i,[num],dtype=DTYPE,initializer=tf.constant_initializer(0.0))

                    conv = tf.nn.conv2d(inp,w,strides=[1,1,1,1],padding='VALID') + b

                    # now max pool
                    conv = tf.nn.max_pool(conv,[1,1,max_chars - width + 1,1],[1,1,1,1],'VALID')
        ...
        if use_proj:
            ...
            with tf.variable_scope("CNN_proj") as scope:
                W_proj_cnn = tf.get_variable('W_proj',[n_filters,projection_dim],initializer=tf.random_normal_initializer(mean=0.0,stddev=np.sqrt(1.0 / n_filters)),dtype=DTYPE)
                b_proj_cnn = tf.get_variable("b_proj",[projection_dim],initializer=tf.constant_initializer(0.0),dtype=DTYPE)
        ...
        if use_highway:
            ...
            for i in range(n_highway):
                with tf.variable_scope("CNN_higt_%s" % i) as scope:
                    W_carry = tf.get_variable('W_carry',[highway_dim,highway_dim],initializer=tf.random_normal_initializer(mean=0.0,stddev=np.sqrt(1.0 / highway_dim)),dtype=DTYPE)
                    b_carry = tf.get_variable("b_carry",[highway_dim],initializer=tf.constant_initializer(-2.0),dtype=DTYPE)

                    W_transform = tf.get_variable('W_transform',[highway_dim,highway_dim],initializer=tf.random_normal_initializer(mean=0.0,stddev=np.sqrt(1.0 / highway_dim)),dtype=DTYPE)
                    b_transform = tf.get_variable("b_transform",[highway_dim],initializer=tf.constant_initializer(0.0),dtype=DTYPE)

                embedding = high(embedding,W_carry,b_carry,W_transform,b_transform)
                if self.bidirectional:
                    embedding_reverse = highway_dim(embedding_reverse,W_carry,b_carry,W_transform,b_transform)
        ...
        if use_proj:
            ...
            self.token_embedding_layers.append(tf.reshape(embedding,[batch_size,unroll_steps,projection_dim]))
    ...
    def _build(self):
        ...
        n_token_vocab = self.options['n_tokens_vocab']
        batch_size = self.options['batch_size']
        unroll_steps = self.options['unroll_steps']
        ...
        for lstm_num,lstm_input in enumerate(lstm_inputs):
            ...
            for i in range(n_lstm_layers):
                if projection_dim < lstm_dim:
                    ...
                    lstm_cell = tf.nn.rnn_cell.LSTMCell(lstm_dim,cell_clip=cell_clip,proj_clip=proj_clip)
            ...
            with tf.control_dependencies([lstm_input]):
                ...
                if self.bidirectional:
                    with tf.variable_scope("RNN_%s" % lstm_num):
                        _lstm_output_unpacked ,final_state = tf.nn.static_rnn(lstm_cell,tf.unstack(lstm_input,axis=1),initial_state = self.init_lstm_state[-1])
                else:
                    _lstm_output_unpacked,final_state = tf.nn.static_rnn(lstm_cell,tf.unstack(lstm_input,axis=1),initial_state = self.init_lstm_state[-1])
                self.final_lstm_state.append(final_state)

            # (batch_size * unroll_steps, 512)
            lstm_output_flat = tf.reshape(tf.stack(_lstm_output_unpacked,axis=1),[-1,projection_dim])

            if self.is_training:
                # add droput to output
    ...
    def _build_loss(self,lstm_outputs):
        ...
        def _get_next_token_placeholders(suffix):
            ...
            id_placeholder = tf.placeholder(DTYPE_INT,shape=(batch_size,unroll_steps),name=name)
        ...
        with tf.variable_scope("softmax"),tf.device('/cpu:0'):
            ...
            if not self.share_embedding_softmax:
                self.softmax_W = tf.get_variable("W",[n_tokens_vocab,softmax_dim],dtype=DTYPE,initializer=softmax_init)
            self.softmax_b = tf.get_variable('b',[n_tokens_vocab],dtype=DTYPE,initializer=tf.constant_initializer(0.0))
        ...
        for id_placeholder,lstm_output_flat in zip(next_ids,lstm_outputs):
            ...
            with tf.control_dependencies([lstm_output_flat]):
                if self.is_training and self.sample_softmax:
                    ...
                    output_scores = tf.matmul(lstm_output_flat,tf.transpose(self.softmax_W)) + self.softmax_b
                    # NOTE: tf.nn.sparse_softmax_cross_entropy_with_logits
                    #   expects unnormalized output since it performs the
                    #   softmax internally
                    losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=output_scores,labels=tf.squeeze(next_token_id_flat,squeeze_dims=[1]))
...
def _get_feed_dict_from_X(X,start,end,model,char_inputs,bidirectional):
    ...
    if bidirectional:
        if not char_inputs:
            feed_dict[model.token_ids_reverse] = X['token_ids_reverse'][start:end]
        else:
            feed_dict[model.token_ids_reverse] = X['tokens_characters_reverse'][start:end]
...
def train(options,data,n_gpus,tf_save_dir,tf_log_dir,restart_ckpt_file =None):
    # not restarting so save the options
    if restart_ckpt_file is None:
        with open(os.path.join(tf_save_dir,'option.json'),'w') as fout:
    ...
    with tf.device("/cpu:0"):
        ...
        tower_grads = []
        models = []
        train_perplexity = tf.get_variable('train_perplexity',[],initializer=tf.constant_initializer(0.0),trainable=False)
        norm_summaries = []
        for k in range(n_gpus):
            with tf.device("/gpu:%d" % k):
                with tf.variable_scope('lm',reuse=k>0):
                    ...
                    models.append(model)
                    grads = opt.compute_gradients(loss*options['unroll_steps'],aggregation_method=tf.AggregationMethod.EXPERIMENTAL_TREE)
        ...
        grads,norm_summaries_ops = clip_grads(grads,options,True,global_step)
        norm_summaries.extend(norm_summaries_ops)
        ...
        his_summary_op = tf.summary.merge(histogram_summaries)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement = True)) as sess:
        ...
        unroll_step = options['unroll_steps']
        n_train_tokens = options.get("n_train_tokens",768648884)
        n_tokens_per_batch = batch_size * unroll_step * n_gpus
        n_batches_per_epoch = int(n_train_tokens/n_tokens_per_batch)
        n_batches_total = options['n_epochs'] * n_batches_per_epoch
        print("Training for % epochs and %s batches" % ( options['n_epochs'],n_batches_total))
        ...
        if char_inputs:
            max_chars = options['char_cnn']['max_character_per_token']

        if not char_inputs:
            feed_dict = { model.token_ids: np.zeros([batch_size,unroll_step],dtype=np.int64)  for model in models }
        else:
            feed_dict = { model.tokens_characters: np.zeros([batch_size,unroll_step,max_chars],dtype=np.int32) for model in models }

        if bidirectional:
            if not char_inputs:
                feed_dict.update({ model.token_ids_reverse : np.zeros([batch_size,unroll_step],dtype=np.int64) for model in models })
            else:
                feed_dict.update({ model.tokens_characters_reverse: np.zeros([batch_size,unroll_step,max_chars],dtype=np.int32) for model in models })

        init_state_values = sess.run(init_state_tensors,feed_dict=feed_dict)

        t1 = time.time()
        data_gen = data.iter_batches(batch_size * n_gpus,unroll_step)
        for batch_no ,batch in enumerate(data_gen,start=1):
            ...
            for k in range(n_gpus):
                ...
                feed_dict.update(_get_feed_dict_from_X(X,start,end,model,char_inputs,bidirectional))
            ...
            if batch_no % 10 != 0:
                ret = sess.run([ train_op,summary_op,train_perplexity] + final_state_tensors,feed_dict=feed_dict)
                ...
                ret = sess.run(
                    [train_op, summary_op, train_perplexity, his_summary_op] +
                    final_state_tensors,
                    feed_dict=feed_dict
                )
            ...
            if (batch_no % 40 == 0 ) or (batch_no == n_batches_total):
                # save the model
...
def clip_grads(grads,options,do_summaries,global_step):
    ...
    def _clip_norms(grad_and_vars,val,name):
        ...
        scaled_val = val
        if do_summaries:
            clipped_tensors,g_norm,so = clip_by_global_norm_summary(grad_tensors,scaled_val,name,vv)
        else:
            so = []
...
def test(options,ckpt_file,data,batch_size=256):
    ...
    if not char_inputs:
        ...
        if bidirectional:
            feed_dict.update({
                model.token_ids_reverse: np.zeros([batch_size,unroll_steps],dtype=np.int64)
            })
    else:
        feed_dict = {
        ...
        if bidirectional:
            feed_dict.update({
                model.tokens_characters_reverse: np.zeros([batch_size,unroll_steps,max_chars],dtype=np.int32)
            })
    ...
    for batch_no,batch in enumerate(data.iter_batches(batch_size,1),start=1):
        ...
        X = batch
        feed_dict = {
             t:v for t,v in zip(init_state_tensors,init_state_values)
        }
        feed_dict.update(_get_feed_dict_from_X(X,0,X['token_ids'].shape[0],model,char_inputs,bidirectional))
        ret = sess.run([model.total_loss,final_state_tensors],feed_dict=feed_dict)

        loss,init_state_values = ret
        batch_losses.append(loss)
        batch_perplexity = np.exp(loss)
        total_loss += loss
        ave_perplexity = np.exp(total_loss /batch_no)
...
def load_options_latest_checkpoint(tf_save_dir):
    ...
    with open(options_file,'r',encoding='utf-8') as fin:
        options = json.load(fin)
    return options,ckpt_file

def load_vocab(vocab_file,max_word_length=None):
    if max_word_length:
        return UnicodeCharsVocabulary(vocab_file,max_word_length,validate_file=True)
    else:
    ...
    with tf.Session(config = config) as sess:
        ...
        with h5py.File(outfile,'w') as fout:
            for v in tf.trainable_variables():
                ...
                print("Saving variable {0} with name {1}".format(v.name,outname))
