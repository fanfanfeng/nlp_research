...
def weight_layers(name,bilm_ops,l2_coef=None,use_top_only=False,do_layer_norm=False):
    ...
    with tf.control_dependencies([lm_embeddings,mask]):
        ...
        def _do_ln(x):
            ...
            mean = tf.reduce_mean(x_masked)/N
        ...
        if use_top_only:
            ...
            normed_weights = tf.split(tf.nn.softmax(W + 1.0/n_lm_layers),n_lm_layers)
            ...
            # get the regularizer
            reg = [ r for r in tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES) if r.name.find("{}_ELMo_W/".format(name)) >= 0 ]
            if len(reg) != 1:
                raise ValueError
        ...
        weight_lm_layers = sum_pieces * gamma
        ret = {'weighted_op':weight_lm_layers,'regularization_op':reg}
