...
class DQN():
    ...
    def create_Q_network(self):
        ...
        with tf.variable_scope('current_net'):
            ...
            # Q Value layer
            self.Q_value = tf.matmul(h_layers,W2) + b2

        with tf.variable_scope('target_net'):
            W1_t = self.weight_variable([self.state_dim,20])
            b1_t = self.bias_variable([20])
            W2_t = self.weight_variable([20,self.action_dim])
            b2_t = self.bias_variable([self.action_dim])

            # hidden layers
            h_layers_t = tf.nn.relu(tf.matmul(self.state_input,W1_t) + b1_t)

            # Q Value layer
            self.target_Q_value = tf.matmul(h_layers_t,W2_t) + b2_t
