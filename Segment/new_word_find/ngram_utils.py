# create by fanfan on 2020/3/12 0012
import re
import types
...
def get_ngram_freq_info(corpus,
                        min_n:int=2,
                        max_n:int=4,
                        chunk_size:int=5000,
                        min_freq:int=2,
    ...
    if isinstance(corpus,types.GeneratorType):
        for corpu_chunk in corpus:
            ngram_freq = _process_corpus_chunk(corpu_chunk)
...
def _ngram_entropy_scorer(parent_ngrams_freq):
    ...
    _parent_ngram_probs = map(lambda x:x/_total_count,parent_ngrams_freq)
    _entropy = sum(map(lambda x: -1 *x * math.log(x,2),_parent_ngram_probs))
    return _entropy
...
def _calc_ngram_entropy(ngram_freq,
    ...
    if CPU_COUNT == 1:
        ...
        for parent_candidate in parent_candidates:
            right_neighors[parent_candidate] = ngram_freq[parent_candidate]
            left_neighbors[parent_candidate[1:]+parent_candidate[0]] = ngram_freq[parent_candidate]

        for target_ngram in target_ngrams:
            try:
                right_neighors_counts = (right_neighors.values(target_ngram))
                right_entropy = _ngram_entropy_scorer(right_neighors_counts)
            except KeyError:
                right_entropy = 0
...
def _calc_ngram_pmi(ngram_freq,ngram_keys,n):
    ...
    for target_ngram in ngram_keys[n]:
        ...
        pmi = math.log(joint_proba/indep_proba,2)
...
def get_scores(corpus,
    ...
    """
        ...
        """
    ngram_freq,ngram_keys = get_ngram_freq_info(corpus,min_n,max_n,chunk_size=chunk_size,
                                                min_freq=min_freq)
    ...
    word_liberalization = lambda le,re:math.log((le*2**re + re*2**le + 0.0000001)/(abs(le-re)+1),1.5)
    word_info_scores = {
