...
class BLSTM_CRF(object):
    def __init__(self,embedded_chars,hidden_unit,cell_type,num_layers,dropout_rate,
        ...
        self.embedded_dims = embedded_chars.shape[-1].value
    ...
    def add_blstm_crf_layer(self,crf_only):
        """
                ...
                """
        if self.is_training:
            # lstm input droput rate i set 0.9 will get best score
    ...
    def project_crf_layer(self,embedding_chars,name=None):
        ...
        with tf.variable_scope('project' if not name else name):
            with tf.variable_scope('logits'):
                W = tf.get_variable("W",shape=[self.embedded_dims,self.num_labels],
                                    dtype=tf.float32,initializer=self.initializers.xavier_initializer())
                b = tf.get_variable('b',shape=[self.num_labels],dtype=tf.float32,
                                    initializer=tf.zeros_initializer())
                output = tf.reshape(self.embedded_chars,shape=[-1,self.embedded_dims])
    ...
    def blstm_layer(self,embedding_chars):
        with tf.variable_scope('rnn_layer'):
            if self.num_layers > 1:
                cell_fw = rnn.MultiRNNCell([self._witch_cell() for _ in range(self.num_layers)])
                cell_bw = rnn.MultiRNNCell([self._witch_cell() for _ in range(self.num_layers)])
            else:
