# create by fanfan on 2019/2/21 0021
import collections
import os
...
from bert_base.train.models import create_model,InputFeatures,InputExample,DataProcessor

__version__ = '0.1.0'
__all__ = ['__version__','DataProcessor','NerProcessor','writer_tokens','convert_single_example',
...
def filed_based_convert_examples_to_features(
    ...
    for(ex_index,example) in enumerate(examples):
        ...
        def create_int_feature(values):
            t = tf.train.Feature(int64_list =tf.train.Int64List(value =list(values)))
            return t
...
def get_last_checkpoint(model_path):
    if not os.path.exists(os.path.join(model_path,'checkpoint')):
        tf.logging.info("checkpoint file not exists:".format(os.path.join(model_path,'checkpoint')))
        return None
    last = None
...
def train(args):
    ...
    if args.do_train and args.do_eval:
        # 加载训练数据
        train_examples = processor.get_train_example(args.data_dir)
        num_train_steps = int(
        ...
        tf.logging.info(' Batch_size = %d',args.batch_size)
        tf.logging.info(" Num steps = %d",num_train_steps)

        eval_examples = processor.get_dev_example(args.data_dir)
    ...
    if args.do_predict:
        ...
        predict_examples = processor.get_test_example(args.data_dir)
        ...
        predict_to_remainder = False
        predict_input_fn = file_based_input_fn_builder(
            input_file=predict_file,
            seq_length=args.max_seq_length,
            is_training=False,
            drop_remainder=predict_to_remainder
        )
