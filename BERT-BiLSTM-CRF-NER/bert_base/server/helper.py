# create by fanfan on 2019/2/28 0028
import argparse
import logging
import os
import sys
...
def set_logger(context,verbose=True):
    if os.name == 'nt':
...
def send_ndarray(src,dest,X,req_id=b'',flags=0,copy=True,track=False):
    ...
    if type(X) == list and type(X[0]) == dict:# 分类for sink发送消息的处理
        ...
        md = dict(dtyp='json',shape=(len(X['pred_label']),1))
    else:
...
def get_args_parser():
    ...
    group1.add_argument('-model_pd_dir',type=str,default=None,
                        help="directory of a pretrained Bert model")
    ...
    group2.add_argument('-max_seq_len',type=int,default=128,
                        help='maxinum length of a sequence')
    ...
    group3.add_argument('-http_max_connect',type=int,default=10,
                        help='maxinum number of concurrent HTTP connections')
    ...
    if os.name == 'nt': # for Windows
        ...
        try:
            ...
            if not os.path.exists(tmp_dir):
                raise ValueError("This directory for sockeds ({}) does not seems to exist.".format(tmp_dir))
            # 随机产生一个
...
def get_benchmark_parser():
    ...
    group.add_argument('-test_client_batch_size',type=int,nargs='*',default=[1,18,256,4096])
    ...
    group.add_argument('-test_num_client',type=int,nargs='*',default=[1,4,6,64])
