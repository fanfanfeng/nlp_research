        ...
        while not self.exit_flag.is_set():
            ...
            if socks.get(receiver) == zmq.POLLIN:
                ...
                if self.args.mode == 'BERT':
                    ...
                    arr_info,arr_val = jsonapi.loads(msg[1]),pickle.loads(msg[2])
                    pending_result[job_info].append((arr_val,partial_id))
                ...
                for job_info,tmp in finished:
                    ...
                    # 因为分类模型带上了分类的概率，所以不能直接返回结果，需要使用json格式的数据进行返回。
            ...
            if socks.get(frontend) == zmq.POLLIN:
                ...
                if msg_type == ServerCommand.new_job:
                    ...
                    time.sleep(0.1)
                    logger.info('send confit \t client %s' % client_addr)
                    sender.send_multipart([client_addr,msg_info,req_id])



class BertWorker(Process):
    def __init__(self,id,args,worker_address_list,sink_address,device_id,graph_path,mode,id2laebl=None):
        super().__init__()
        self.worker_id = id
