# create by fanfan on 2019/3/1 0001
import contextlib
import json
import os
...
__all__ = [ 'optimize_bert_graph', 'optimize_ner_model', 'optimize_class_model']
    ...
    try:
        ...
        with jit_scope():
            ...
            masked_reduce_mean = lambda x,m:tf.reduce_sum(mul_mask(x,m),axis=1)/(
                tf.reduce_mean(m,axis=1,keepdims=True) + 1e-10)
        ...
        with tf.Session(config=config) as sess:
            ...
            dtypes = [n.dtyp for n in input_tensors]
        ...
        logger.info("write graph to a temp file : %s" % args.model_pd_dir)
        with tf.gfile.GFile(pb_file,'wb') as f:
            f.write(tmp_g.SerializeToString())
    except Exception:
        logger.error("fail to optimizer the graph!",exc_info=True)
...
def convert_variables_to_constants(sess,
    ...
    for node in inference_graph.node:
        if node.op in ['Variable',"VariableV2",'VarHandleOp']:
            ...
            if ((variable_names_whitelist is not None and variable_name not in variable_names_whitelist) or
                    (variable_names_blacklist is not None and variable_name in variable_names_blacklist)):
                continue
    ...
    for input_node in inference_graph.node:
        ...
        if input_node.name in found_variables:
            ...
            if use_fp16 and dtype.dtype == types_pb2.DT_FLOAT:
                output_node.attr['value'].CopyFrom(
                    attr_value_pb2.AttrValue(tensor=tensor_util.make_tensor_proto(data.asType('float16'),
...
def optimize_ner_model(args,num_labels,logger=None):
    """
        ...
        """
    if not logger:
        logger = set_logger(colored('NER_MODEL,Loadding...','cyan'),args.verbose)
...
def optimize_class_model(args,num_labels,logger=None):
    """
        ...
        """
    if not logger:
        logger = set_logger(colored('CLASSIFICATION_MODEL,Loadding...','cyan'),args.verbose)
    ...
    try:
        ...
        with graph.as_default():
            with tf.Session() as sess:
                ...
                _,_,_,probabilities = create_classification_model(
                    bert_config=bert_config,
                    is_training=False,
                    input_ids=input_ids,
                    input_mask=input_mask,
