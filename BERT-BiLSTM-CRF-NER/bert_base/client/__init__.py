# create by fanfan on 2019/3/4 0004
import sys
import threading
import time
import uuid
...
class BertClient(object):
    ...
    def _timeout(func):
        def arg_wrapper(self,*args,**kwargs):
            ...
            try:
                ...
                t_e = TimeoutError('no response from the server (with "timeout"=%d ms), please check the following:'
    ...
    def server_status(self):
        """
                ...
                """
        self.receiver.setsockopt(zmq.RCVTIEMO,self.timeout)
        self._send(b'SHOW_CONFIG')
        return jsonapi.loads(self._recv().content[1])

    @_timeout
    def encode(self,textx,blocking=True,is_tokenized=False):
        """ Encode a list of strings to a list of vectors
                ...
                """
        if is_tokenized:
            self._check_input_lst_lst_str(textx)
        else:
            self._check_input_lst_str(textx)

        if self.length_limit and not self._check_length(textx,self.length_limit,is_tokenized):
    ...
    def _check_input_lst_str(texts):
        ...
        if not len(texts):
            raise  ValueError(""
                              "%s must be a non-empty list ,bu received %s with %d elements" % (texts,type(texts),len(texts)))
        ...
        for idx,s in enumerate(texts):
            ...
            if not s.strip():
                raise ValueError("all elements in the list must be non-empty string ,but element %d is %s" % (idx,repr(s)))
...
class ConcurrentBertClient(BertClient):
    def __init__(self,max_concurrentcy= 10,**kwargs):
        ...
        self.avalable_bc = [BertClient(**kwargs) for _ in range(max_concurrentcy)]
        self.max_concurrency = max_concurrentcy

    def close(self):
        for bc in self.avalable_bc:
            bc.close()
