# create by fanfan on 2019/3/11 0011
...
model_dir =  r'E:\git-project\nlp_research\BERT-BiLSTM-CRF-NER\output'
...
def convert_single_example(ex_index,example,label_list,max_seq_length,tokenizer,mode):
    ...
    # 保存label->index 的map
    if not os.path.exists(os.path.join(model_dir,'lael2id.pkl')):
    ...
    assert len(segmetn_ids) == max_seq_length
    assert len(label_ids) == max_seq_length

    feature = InputFeatures(
        input_ids = input_ids,
        input_mask = input_mask,
        segment_ids = segmetn_ids,
        label_ids = label_ids
    )
