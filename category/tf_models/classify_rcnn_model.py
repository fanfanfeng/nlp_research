...
class ClassifyRcnnModel(BaseClassifyModel):
    ...
    def classify_layer(self, input_embedding,dropout,real_sentence_length):
        ...
        with tf.variable_scope("context"):
            ...
            contextLeft = tf.concat([tf.zeros(shape),output_fw[:,:-1]],axis=1,name='context_left')
            contextRight = tf.concat([output_bw[:,1:],tf.zeros(shape)],axis=1,name='contextRight')

        # 将前向，后向的输出和最早的词向量拼接在一起得到最终的词表征
        with tf.variable_scope('wordRepresentation'):
            wordRepre = tf.concat([contextLeft,input_embedding,contextRight],axis=1)

        with tf.variable_scope('textRepresentation'):
            textRepre = tf.layers.dense(wordRepre,self.params.hidden_size,activation=tf.tanh)
