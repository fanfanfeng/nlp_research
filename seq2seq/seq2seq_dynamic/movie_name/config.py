...
source_vocabulary = 'data/vocab60000.in' # source 的词库存储路径
target_vocabulary = 'data/vocab60000.in' # target的词库存储路径,用于训练对话，所以词库是同一个


# tensorflow网络设置参数
cell_type =  'lstm'             # 编码跟解码截断的RNN cell类型 , 默认: lstm
attention_type =  'bahdanau'    # 注意力机制类型: (bahdanau, luong), 默认: bahdanau
hidden_units =  128            # 隐层神经元个数
depth = 2                       # 神经网络的层数
embedding_size =  100           # 编码跟解码输出的向量Embedding dimensions
num_encoder_symbols = 60000     # source 的词库大小
num_decoder_symbols = 60000    # target 的词库大小
...
model_name = 'video_name.ckpt'        # 模型保存的名称
