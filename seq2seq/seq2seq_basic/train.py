# create by fanfan on 2018/3/1 0001
...
from seq2seq.seq2seq_basic import seq2seq_model
import numpy as np
import time
import math
...
def train():
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,)) as sess:
        ...
        with tf.device('/gpu:0'):
            model_obj = seq2seq_model.Seq2seqModel()
            model_obj.build_network()
            model_obj._init_optimizer()
            tf_saver = tf.train.Saver()
            model_obj.model_restore(sess,tf_saver)
            print("开始训练")
            loss = 0.0
            ...
            for epoch_id  in range(config.max_epochs):
                for step, train_batch in enumerate(tran_batch_manager.iterbatch()):
                    ...
                    step_loss = model_obj.train(sess,encoder_inputs=train_batch['encode'],
                    ...
                    if (model_obj.global_step.eval() +1) % config.display_freq == 0:
                        if loss < best_loss:
                            ...
                            tf_saver.save(sess, checkpoint_path, global_step=model_obj.global_step)
                    ...
                    if (model_obj.global_step.eval() +1) % config.valid_freq == 0:
                        ...
                        for test_batch in test_batch_manager.iterbatch():
                            step_loss = model_obj.eval(sess,encoder_inputs=test_batch['encode'],
