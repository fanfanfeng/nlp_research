...
from dialog_system.end2end_memn2n.utils import ProcessBar

class MemN2n(object):
    def __init__(self,sess,nwords,max_words,max_sentence):
        self.nwords = nwords
        ...
        self.query = tf.placeholder(tf.int32,[None,self.max_words],name='query')
    ...
    def build_model(self):
        ...
        self.loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=a_hat,labels=self.target)
    ...
    def build_memory(self):
        ...
        # [batch_size, mem_size, max_length, edim]
        A_embedding = tf.nn.embedding_lookup(A,self.context)
        # [batch_size, mem_size, edim]
        A_embedding = tf.reduce_sum(A_embedding,axis=2)
        # [batch_size, mem_size, edim]
        T_A_embedding = tf.nn.embedding_lookup(T_A,self.time)
        # [batch_size, mem_size, edim]
        A_in = tf.add(A_embedding,T_A_embedding)

        # [batch_size, mem_size, max_length, edim]
        C_embedding = tf.nn.embedding_lookup(C,self.context)
        # [batch_size, mem_size, edim]
        C_embedding = tf.reduce_sum(C_embedding,axis=2)
        # [batch_size, mem_size, edim]
        T_C_embedding = tf.nn.embedding_lookup(T_C,self.time)
        # [batch_size, mem_size, edim]
        C_in = tf.add(C_embedding,T_C_embedding)

        # [batch_size, max_length, edim]
        query_embedding = tf.nn.embedding_lookup(B,self.query)
        # [batch_size, edim]
        query_embedding = tf.reduce_sum(query_embedding,axis=1)
        self.hid.append(query_embedding)


        for h in range(self.nhop):
            # [batch_size, edim] ==> [batch_size, 1, edim]
            q3dim = tf.reshape(self.hid[-1],[-1,1,self.edim])
            # [batch_size, 1, edim] X [batch_size, edim, mem_size]
            p3dim = tf.matmul(q3dim,A_in,transpose_b=True)
            # [batch_size, mem_size]
            p2dim = tf.reshape(p3dim,[-1,self.mem_size])
            ...
            p3dim = tf.reshape(p,[-1,1,self.mem_size])
            # [batch_size, 1, mem_size] X [batch_size, mem_size, edim]
            o3dim = tf.matmul(p3dim,C_in)
            # [batch_size, edim]
            o2dim = tf.reshape(o3dim,[-1,self.edim])

            # [batch_size, edim]
            a = tf.add(o2dim,self.hid[-1])
            # [input, a_1, a_2, ..., a_nhop]
            self.hid.append(a)
    ...
    def train(self,train_stories,train_questions):
        ...
        for idx in range(N):
            ...
            _,loss,self.step = self.sess.run([self.optim,self.loss,self.global_step],feed_dict={self.query:query,self.time:time,self.target:target,self.context:context})
            cost += np.sum(loss)
    ...
    def run(self,train_stories,train_questions,test_stories,test_questions):
        if not self.is_test:
            for idx in range(self.nepoch):
                ...
                test_loss = np.sum(self.test(test_stories,test_questions))
