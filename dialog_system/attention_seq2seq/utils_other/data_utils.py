...
def process_batch_data(batch_lu, batch_ru, word_dict):
    ...
    for lu, ru in zip(batch_lu, batch_ru):
        ...
        ru_in = [word_dict[GO]] + ru + [word_dict[EOS]] * (max_ru_len -1 - len(ru))  # add GO for decoder input
        # add PAD and EOS at the end for decoder output
        ru_out = ru + [word_dict[EOS]] + [word_dict[EOS]] * (max_ru_len -1 - len(ru))  # add EOS for decoder output
        b_lu.append(lu)
