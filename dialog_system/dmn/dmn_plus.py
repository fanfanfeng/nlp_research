...
from dialog_system.dmn.attention_gru_cell import  AttentionGRUCell
from dialog_system.dmn import babi_input
class Config(object):
...
class DNM_PLUS(object):
    ...
    def get_question_representation(self,embeddings):
        ...
        _,q_vec = tf.nn.dynamic_rnn(gru_cell,questions,dtype=tf.float32,sequence_length=self.question_len_placeholder)
    ...
    def get_input_representation(self,embeddings):
        ...
        # use position encoding to get sentence representation
    ...
    def get_attention(self,q_vec,prev_memory,fact_vec,reuse):
        ...
        attention = tf.contrib.layers.fully_connected(features_vec,self.config.embed_size,activation_fn = tf.nn.tanh,reuse =reuse,scope='fc1')
    ...
    def add_answer_module(self,rnn_output,q_vec):
        ...
        run_output = tf.nn.dropout(rnn_output,self.dropout_placeholder)
        output = tf.layers.dense(tf.concat([rnn_output,q_vec],1),self.vocab_size,activation=None)
    ...
    def inference(self):
        ...
        with tf.variable_scope('memory',initializer=tf.contrib.layers.xavier_initializer()):
            ...
            for i in range(self.config.num_hops):
                ...
                episode = self.generate_episode(prev_meory,q_vec,fact_vecs,i)
                with tf.variable_scope('hop_%d' % i):
                    prev_meory = tf.layers.dense(tf.concat([prev_meory,episode,q_vec],1),self.config.hidden_size,activation=tf.nn.relu)
    ...
    def run_epoch(self,session,data,num_epoch=0,train_writer=None,train_op=None,verbose=2,train=False):
        ...
        shuffle_index = np.random.permutation(len(data[0]))
        qp,ip,ql,il,im,a,r = data
        qp,ip,ql,il,im,a,r = qp[shuffle_index],ip[shuffle_index],ql[shuffle_index],il[shuffle_index],im[shuffle_index],a[shuffle_index],r[shuffle_index]
