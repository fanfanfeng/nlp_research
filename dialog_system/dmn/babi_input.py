# create by fanfan on 2018/10/19 0019
import os
import numpy as np
input_mask_mode = 'sentence'

def init_babi(fname):
    print("从%s中加载训练集" % fname)
    tasks = []
    task = None
...
def get_babi_raw(id, test_id):
    ...
    DATA_ROOT = r'E:\tensorflow_data\babi'
    babi_test_name = babi_map[test_id]
    babi_train_raw = init_babi(os.path.join(DATA_ROOT,'en-10k/%s_train.txt' % babi_name))
    babi_test_raw = init_babi(os.path.join(DATA_ROOT,'en-10k/%s_test.txt' % babi_test_name))
...
def create_vector(word,word2vec,word_vecter_size,silent=True):
    vector = np.random.uniform(0.0,1.0,(word_vecter_size,))
    word2vec[word] = vector
    if not silent:
        print("data_utils.py::create_vector => %s is missing" % word)
    return vector
...
def process_word(word,word2vec,vocab,ivocab,word_vector_size,to_return='word2vec',silent=True):
    ...
    if to_return == 'word2vec':
        ...
        raise  Exception("onehot is not implemented yet")


def process_input(data_raw,floatX,word2vec,vocab,ivocab,embed_size,split_senteces = False):
    questions = []
    inputs = []
    answers =[]
    input_masks = []
    ...
    for x in data_raw:
        ...
        if split_senteces:
            inp_vector = [[process_word(word=w,word2vec=word2vec,vocab=vocab,ivocab=ivocab,word_vector_size=embed_size,to_return='index') for w in s] for s in inp]
        else:
            inp_vector = [ process_word(word=w,word2vec=word2vec,vocab = vocab,ivocab = ivocab,word_vector_size=embed_size,to_return='index') for w in inp]

        q_vector =[process_word(word=w,word2vec=word2vec,vocab=vocab,ivocab=ivocab,word_vector_size=embed_size,to_return='index') for w in q]
